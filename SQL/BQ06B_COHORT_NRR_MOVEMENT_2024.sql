-- ============================================================
-- BQ06B: COHORT_NRR_MOVEMENT_2024
-- Input: BQ06A_CUSTOMER_NRR_MOVEMENT_2024
--        One row per customer with movement amounts and label.
-- Grain: one row per paid signup cohort month
-- ============================================================

CREATE OR REPLACE TABLE LEDGERLY.ANALYTICS.BQ06B_COHORT_NRR_MOVEMENT_2024 AS


-- Step 1: movement_filtered keeps the cohort movement columns from BQ06A.
-- BQ06A already produced one clean movement row per customer.
-- movement_filtered starts there and keeps only the columns BQ06B needs: paid signup cohort month,
...
(
    ...
    FROM LEDGERLY.ANALYTICS.BQ06A_CUSTOMER_NRR_MOVEMENT_2024
),
...
-- Since BQ06B uses true percent format, 126.59 means 126.59%.
