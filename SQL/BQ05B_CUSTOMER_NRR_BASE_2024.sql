...
-- ============================================================
-- BQ05B: CUSTOMER_NRR_BASE_2024
-- Purpose: Join BQ05A to BQ03A (aggregated to customer level)
...
-- To get there, we aggregate BQ03A by CUSTOMER_ID and join to BQ05A — and that is BQ05B.
...
CREATE OR REPLACE TABLE LEDGERLY.ANALYTICS.BQ05B_CUSTOMER_NRR_BASE_2024 AS
...
-- Join BQ05A_PAID_SIGNUP_COHORT_BY_CUSTOMER to the customer-level starting MRR table on CUSTOMER_ID. 
-- This adds each customer’s STARTING_MRR_CENTS to their paid signup cohort. 
-- The customer cohort comes from BQ05A, and the starting MRR comes from BQ03A.

cohort_mrr_start_join AS
(
    -- Step 3: Join BQ05A to starting MRR on CUSTOMER_ID
    SELECT
        c.CUSTOMER_ID,
        c.PAID_SIGNUP_COHORT_MONTH,
        COALESCE(m.STARTING_MRR_CENTS, 0) AS STARTING_MRR_CENTS

    FROM LEDGERLY.ANALYTICS.BQ05A_PAID_SIGNUP_COHORT_BY_CUSTOMER AS c
...
-- BQ05C will convert that missing ending MRR to zero when it sums cohort totals.
