...
-- ============================================================
-- BQ06C: COHORT_NRR_2024
-- Purpose: Aggregate BQ06B to one row per cohort month.
--          Calculate NRR and rank cohorts strongest to weakest.
-- Grain: one row per paid signup cohort month
-- ============================================================

CREATE OR REPLACE TABLE LEDGERLY.ANALYTICS.BQ06C_COHORT_NRR_2024 AS


-- Step 1: cohort_mrr_aggregated
-- Start from BQ06B_CUSTOMER_NRR_BASE_2024 because BQ06B already has one row per customer in the 2024 NRR base. 
...
-- This final table answers BQ06: which paid signup cohorts had the strongest and weakest 2024 year-end NRR.
