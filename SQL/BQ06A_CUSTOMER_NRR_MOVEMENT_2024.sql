...
-- BQ05B_CUSTOMER_NRR_BASE_2024 is the customer-level NRR base.
-- It has:
...
-- CREATE OR REPLACE TABLE LEDGERLY.ANALYTICS.BQ06A_CUSTOMER_NRR_MOVEMENT_2024 AS

-- Step 1: nrr_base_filtered keeps the customer-level NRR base columns from BQ05B.

-- BQ06 needs to explain what happened to each customer's recurring revenue across 2024.
-- That information already lives in BQ05B at the right grain — one row per customer —
...
-- This keeps the table focused because BQ06 is not rebuilding the NRR base.
...
-- a negative number, and the cohort-level sum in BQ06B silently drops their lost MRR.
...
(
    SELECT
        ...
        -- Contraction: ending MRR is lower than starting MRR but still above zero.
        CASE
            WHEN ENDING_MRR_CENTS < STARTING_MRR_CENTS
        ...
        -- Net MRR change: COALESCE makes churned customers contribute a negative amount instead of NULL.
...
-- The movement label lets BQ06B count how many customers caused each type of movement,
